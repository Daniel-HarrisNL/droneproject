...
TELLO_SIMPLE
    ...
    Written by Roy Brushett while recovering from surgery.
'''
import socket
...
class Tello:
    ...
    def disconnect(self):
        ...
        Should be called at the end of your program or when you're otherwise finished sending commands.
        Returns nothing.
        '''
    ...
    def connect_and_initialize(self, ip=None, port=None):
        ...
        This allows it to receive other commands, and only has to be done once.

        This method returns the status message back from the drone (either an error or the string 'ok'), as well as the
    ...
    def initialize_command_mode(self):
        ...
        '''
        return send_initialize_command_mode_command_to_socket(self.conn)

    def emergency(self):
        '''
        Sends the emergency command to the drone, forcing it to kill power to the motors immediately.
    ...
    def send_command(self, command_string):
        ...
        Sends a command string to the drone and returns it's response, as well as the number of bytes sent.
           ...
           return - A tuple with two items:
            Item 1: Whatever was received from the drone
...
def send_emergency_command_to_socket(soc, send_command_to_socket=send_command_to_socket):
    ...
    This is a helper function that implements the behavior for a method in the Tello class.
    This should not be used on it's own, use the Tello class instead.
    Sends the emergency command to the drone and returns it's response, as well as the number of bytes sent.
       soc - A socket like object that represents the connection to tello.
       send_command_to_socket (optional) - You can override the default send command if you'd like.
       return - ('ok' | error message, number_of_bytes)
    '''
...
INITIALIZE_COMMAND_COMMAND_STRING = "command"


def send_initialize_command_mode_command_to_socket(soc, send_command_to_socket=send_command_to_socket):
    '''
    This is a helper function that implements the behavior for a method in the Tello class.
    This should not be used on it's own, use the Tello class instead.
    Sends the initialization command to the drone and returns it's response, as well as the number of bytes sent.
       ...
       return - ('ok' | error message, number_of_bytes)
    '''
    return send_command_to_socket(soc, INITIALIZE_COMMAND_COMMAND_STRING)
